...
// -- Drive Config -----------------------------------------------------------
const ROOT_FOLDER_ID = '1XNzrsiVf93hikgIrzphSRdUc_44kaUuK'; // PLC Dealer Agreements

const DEALER_FOLDER_NAMES = {
  'energypure':        'Energy Pure',
  'energy pure':       'Energy Pure',
  'stakenterprisesllc':'Stak Enterprises',
  'stak enterprises':  'Stak Enterprises',
  'stak':              'Stak Enterprises',
  'voltaic':           'Voltaic',
  'nomad12':           'Voltaic',
  'plc':               'PLC',
  'pioneer legal':     'PLC',
...
function getDealerFolderName(companyName) {
  if (!companyName) return 'PLC';
  const key = companyName.trim().toLowerCase().replace(/[^a-z0-9 ]/g, '').replace(/\s+/g, ' ');
  for (const [pattern, folder] of Object.entries(DEALER_FOLDER_NAMES)) {
    if (key.includes(pattern)) return folder;
  }
  return companyName.trim(); // fallback: use company name as-is
}

// -- Save to Google Drive ---------------------------------------------------
async function saveToDrive(pdfBytes, fileName, dealerFolderName) {
  try {
    const accessToken = await getGoogleAccessToken();
    if (!accessToken) {
      console.log('No Drive credentials -- skipping Drive save');
      return null;
    }

    // Find or create dealer subfolder
    const dealerFolderId = await findOrCreateFolder(accessToken, ROOT_FOLDER_ID, dealerFolderName);
    if (!dealerFolderId) return null;

    // Upload PDF
    const fileId = await uploadFileToDrive(accessToken, dealerFolderId, fileName, pdfBytes);
    return fileId;
  } catch (err) {
...
async function findOrCreateFolder(token, parentId, name) {
  ...
  const found = await new Promise((resolve) => {
    const req = https.request({
      ...
      path: `/drive/v3/files?q=${query}&fields=files(id)`,
      method: 'GET',
      ...
      res.on('end', () => {
        try {
          ...
          resolve(files && files.length > 0 ? files[0].id : null);
        } catch(e) { resolve(null); }
      });
    });
    req.on('error', () => resolve(null));
  ...
  return new Promise((resolve) => {
    const body = JSON.stringify({ name, mimeType: 'application/vnd.google-apps.folder', parents: [parentId] });
    const req = https.request({
      hostname: 'www.googleapis.com',
...
async function uploadFileToDrive(token, folderId, fileName, pdfBytes) {
  const boundary = 'plc_boundary_' + Date.now();
...
function drawSection(page, title, font, boldFont, fontSize, x, y, pageWidth, margin) {
  ...
  page.drawText(title.toUpperCase(), { x: margin + 6, y: y + 1, size: 8, font: boldFont, color: WHITE });
  return y - 22;
}
...
async function buildPDF(payload) {
  ...
  function newPage() {
    ...
    y = pageHeight - margin;
    page.drawText(safe('Pioneer Legal Consulting, LLC  --  445 133rd Ave, Wayland, MI 49348  --  wsnyder@plc-us.com'), { x: margin, y: 30, size: 7, font, color: GRAY });
    const pageCount = pdfDoc.getPageCount();
    page.drawText(`Page ${pageCount}`, { x: pageWidth - margin - 40, y: 30, size: 7, font, color: GRAY });
  }

  function checkY(needed = lineH * 3) { if (y < margin + needed) newPage(); }

  function drawLine(text, opts = {}) {
    checkY();
    const { size = bodySize, f = font, color = BLACK, indent = 0, maxWidth = contentWidth - indent, center = false } = opts;
  ...
  drawLine('445 133rd Ave  --  Wayland, MI 49348  --  wsnyder@plc-us.com  --  (313) 635-3709', { size: 7.5, color: GRAY, center: true });
  spacer(0.5);
  ...
  docTitles.forEach(title => { drawLine(title, { size: 12, f: boldFont, color: NAVY, center: true }); });
  ...
  if (payload.docs.dealer) {
    ...
    drawLine('2.1 Wholesale Service Fee: $5,500.00 per Client. Dealer is solely responsible for all Service Fees. Clients have no payment obligation to PLC under this Agreement.', { size: bodySize, indent: 8, f: boldFont });
    drawLine('2.2 Payment Before Work Begins. PLC requires payment of the full $5,500.00 Service Fee before any work is initiated. This is a firm, non-negotiable condition of engagement.', { size: bodySize, indent: 8 });
    drawLine("2.3 Accepted Payment Methods. PLC accepts wire transfer, Zelle, or ACH only. PLC does not accept credit cards or any payment method that incurs a processing fee on PLC's end. If any payment method results in a processing fee, that fee is borne entirely by Dealer and PLC's full $5,500.00 wholesale fee must be received net of all fees. The amount received by PLC must equal the full $5,500.00 regardless of any fees charged by the payment processor.", { size: bodySize, indent: 8 });
    drawLine("2.4 Klarna/Third-Party Financing. Klarna acceptance/denial is solely Klarna's determination. Klarna/Stripe processing fees are borne entirely by Dealer. PLC's $5,500.00 fee is owed regardless of fees imposed. Dealer's net proceeds are paid only after funds clear PLC's deposit account.", { size: bodySize, indent: 8 });
    ...
    drawLine("2.11 Chargeback Protection. Dealer remains fully responsible for PLC's $5,500.00 fee regardless of any client chargeback, refund demand, financing reversal, reserve hold, or client default.", { size: bodySize, indent: 8 });
    spacer(0.5);
    ...
    drawLine("3.3 Dealer Retains Full Financial Risk. PLC's $5,500.00 fee is owed in full regardless of whether Dealer collected from any Client or the outcome of any financing.", { size: bodySize, indent: 8 });
    ...
    page.drawText('PERSONAL GUARANTEE OF PAYMENT', { x: margin + 10, y: y - 8, size: 8.5, font: boldFont, color: NAVY });
    ...
    let gy = y - 20;
    gLines.forEach(line => { page.drawText(line, { x: margin + 10, y: gy, size: smallSize, font, color: BLACK }); gy -= 11; });
    y = gy - 10;
  }
  ...
  if (payload.docs.dealer && isEnergyPure) {
    ...
    drawLine("EP-6.3 Payment Obligations Apply from Day One. All Client submissions during the launch period are subject to the full $5,500.00 Service Fee, 48-hour payment requirement, and all other payment terms of this Agreement.", { size: bodySize, indent: 8 });
    spacer(0.8);
  }
  ...
  page.drawText(payload.docs.dealer ? 'DEALER / VENDOR:' : 'REPRESENTATIVE:', { x: col1X, y: sigY, size: 8, font: boldFont, color: NAVY });
  ...
  try {
    ...
    page.drawImage(sigImg, { x: col1X, y: sigY - 55, width: Math.min(sigDims.width, colW), height: Math.min(sigDims.height, 50) });
  } catch(e) {
  ...
  page.drawText(safe(`${payload.signerTitle || ''}${payload.companyName ? '  |  ' + payload.companyName : ''}`), { x: col1X, y: sigY - 84, size: 8, font, color: GRAY });
  ...
  y -= 8;
  drawLine('This document was electronically signed and timestamped. Pioneer Legal Consulting, LLC maintains a record of this execution.', { size: 7, color: GRAY, center: true });
  drawLine(`Submitted: ${new Date().toISOString()} -- Signer IP logged for verification`, { size: 7, color: GRAY, center: true });

  const pages = pdfDoc.getPages();
  pages[0].drawText('Pioneer Legal Consulting, LLC  --  445 133rd Ave, Wayland, MI 49348  --  wsnyder@plc-us.com', { x: margin, y: 30, size: 7, font, color: GRAY });
...
async function sendEmails(pdfBytes, payload) {
  ...
  await transporter.sendMail({
    ...
    html: `<div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;"><div style="background:#0A1F44;padding:24px 32px;border-bottom:3px solid #C9A84C;"><div style="color:#fff;font-size:18px;font-weight:bold;">Pioneer Legal Consulting, LLC</div><div style="color:#C9A84C;font-size:11px;letter-spacing:2px;text-transform:uppercase;margin-top:4px;">Electronic Signing Confirmation</div></div><div style="padding:32px;background:#fff;border:1px solid #e5e7eb;"><p style="font-size:15px;color:#1a1a2e;margin-bottom:16px;">Dear ${payload.signerName},</p><p style="color:#374151;line-height:1.6;margin-bottom:16px;">Thank you for signing the <strong>${docLabel}</strong> with Pioneer Legal Consulting, LLC. Your signed agreement is attached.</p><div style="background:#0A1F44;color:#fff;border-radius:8px;padding:16px;font-size:13px;"><strong>Wayne Snyder</strong> -- Director of Operations<br>wsnyder@plc-us.com &nbsp;--&nbsp; (313) 635-3709<br>445 133rd Ave, Wayland, MI 49348</div></div></div>`,
  ...
  await transporter.sendMail({
    ...
    html: `<div style="font-family:Arial,sans-serif;max-width:560px;"><div style="background:#0A1F44;padding:20px 28px;border-bottom:3px solid #C9A84C;"><div style="color:#C9A84C;font-weight:bold;font-size:14px;">New Agreement Signed</div></div><div style="padding:24px;background:#fff;border:1px solid #e5e7eb;"><table style="width:100%;font-size:13px;border-collapse:collapse;"><tr><td style="padding:6px 0;color:#6b7280;width:140px;">Document(s)</td><td style="padding:6px 0;font-weight:600;color:#1a1a2e;">${docLabel}</td></tr><tr><td style="padding:6px 0;color:#6b7280;">Signer Name</td><td style="padding:6px 0;color:#1a1a2e;">${payload.signerName}</td></tr><tr><td style="padding:6px 0;color:#6b7280;">Title</td><td style="padding:6px 0;color:#1a1a2e;">${payload.signerTitle || '--'}</td></tr><tr><td style="padding:6px 0;color:#6b7280;">Date of Birth</td><td style="padding:6px 0;color:#1a1a2e;">${payload.signerDOB || '--'}</td></tr><tr><td style="padding:6px 0;color:#6b7280;">Company</td><td style="padding:6px 0;color:#1a1a2e;">${payload.companyName || '--'}</td></tr><tr><td style="padding:6px 0;color:#6b7280;">State</td><td style="padding:6px 0;color:#1a1a2e;">${payload.companyState || '--'}</td></tr><tr><td style="padding:6px 0;color:#6b7280;">Business Address</td><td style="padding:6px 0;color:#1a1a2e;">${payload.companyAddress || '--'}</td></tr><tr><td style="padding:6px 0;color:#6b7280;">Home/Alt Address</td><td style="padding:6px 0;color:#1a1a2e;">${payload.companyAddress2 || '--'}</td></tr><tr><td style="padding:6px 0;color:#6b7280;">Email</td><td style="padding:6px 0;color:#1a1a2e;">${payload.signerEmail}</td></tr><tr><td style="padding:6px 0;color:#6b7280;">Primary Phone</td><td style="padding:6px 0;color:#1a1a2e;">${payload.signerPhone}</td></tr><tr><td style="padding:6px 0;color:#6b7280;">Secondary Phone</td><td style="padding:6px 0;color:#1a1a2e;">${payload.signerPhone2 || '--'}</td></tr><tr><td style="padding:6px 0;color:#6b7280;">Date Signed</td><td style="padding:6px 0;color:#1a1a2e;">${payload.signedDate}</td></tr><tr><td style="padding:6px 0;color:#6b7280;">Submitted</td><td style="padding:6px 0;color:#1a1a2e;">${new Date().toISOString()}</td></tr></table></div></div>`,
    attachments: [pdfAttachment],
  });
}
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Save to Drive (non-blocking -- won't fail submission if Drive save fails)
    const dealerFolderName = getDealerFolderName(payload.companyName);
    const fileName = `PLC_Signed_${payload.docs.dealer ? 'Dealer' : ''}${payload.docs.dealer && payload.docs.rep ? '_' : ''}${payload.docs.rep ? 'Rep' : ''}_${payload.signerName.replace(/\s+/g,'_')}_${new Date().toISOString().slice(0,10)}.pdf`;
    saveToDrive(pdfBytes, fileName, dealerFolderName).catch(err => console.error('Drive save failed:', err.message));
